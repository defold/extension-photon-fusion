...
namespace FusionCore::Notify
{
	...
	public:
		...
		// data.Ptr may alias a stack buffer in the caller (Connection::Send keep-alive,
		// Connection::SendFragment). implementations MUST consume the bytes synchronously
		// before returning — copy them if you need to defer/queue.
