...
namespace PhotonCommon
{
	...
	public:
		...
		Subscription Subscribe(Callback cb)
		{
			...
			if (dispatchDepth > 0)
			{
				deferred.push_back([this, cb = std::move(cb), state, si]() mutable {
					slots[si].packedIndex = static_cast<uint32_t>(packed.size());
	...
	private:
		...
		{
			Broadcaster* owner;
			explicit DispatchGuard(Broadcaster* o) : owner(o) { ++owner->dispatchDepth; }
			~DispatchGuard() { owner->EndDispatch(); }
