...
namespace FusionCore
{
	...
	{
		return CRC64(&data, sizeof(T));
	}

	template <typename T, std::enable_if_t<!std::is_pointer_v<T>, bool> = true>
	uint64_t CRC64(const uint64_t crc, T data)
	{
		return CRC64(crc, &data, sizeof(T));
	}
	...
	inline uint32_t QuaternionCompress(T x, T y, T z, T w)
	{
		...
		return static_cast<uint32_t>(a * ENCODER + HALF_ENCODED) | static_cast<uint32_t>(b * ENCODER + HALF_ENCODED) << 10 | static_cast<uint32_t>(c * ENCODER + HALF_ENCODED) << 20 | static_cast<uint32_t>(biggestAxis) << 30;
	}
	...
	inline void QuaternionDecompress(uint32_t buffer, T& outX, T& outY, T& outZ, T& outW)
	{
		...
		{
			case 0:
				outX = static_cast<T>(d);
				outY = static_cast<T>(a);
				outZ = static_cast<T>(b);
				outW = static_cast<T>(c);
				break;
			case 1:
				outX = static_cast<T>(a);
				outY = static_cast<T>(d);
				outZ = static_cast<T>(b);
				outW = static_cast<T>(c);
				break;
			case 2:
				outX = static_cast<T>(a);
				outY = static_cast<T>(b);
				outZ = static_cast<T>(d);
				outW = static_cast<T>(c);
				break;
			default:
				outX = static_cast<T>(a);
				outY = static_cast<T>(b);
				outZ = static_cast<T>(c);
				outW = static_cast<T>(d);
				break;
		}
	}
} // namespace FusionCore
