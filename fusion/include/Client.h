...
namespace FusionCore
{
	...
	class Client
	{
		...
		PhotonMatchmaking::RealtimeClient* realtimeClient{nullptr};
		...
		struct DeferredEvent
		{
			...
			DeferredEvent(DeferredEvent&&)                 = default;
			DeferredEvent& operator=(DeferredEvent&&)      = default;
		};

		std::deque<DeferredEvent> _deferredEvents{};
		...
		void InvokeDeferredEvents();
		void FreeDeferredEvents();
	...
	public:
		...
		PhotonCommon::Broadcaster<void(PhotonCommon::StringType message, ForcedDisconnectReason reason)> OnForcedDisconnect;
		...
		// The integration answers asynchronously by calling
		// Client::RespondToOwnershipRequest(obj, requester, granted) — pass the
		// PlayerId from the callback so concurrent requests from different
		// players are each answered to the correct target. The response is
		// validated and forwarded by the plugin and surfaces on the requester
		// via OnOwnershipResponse.
		PhotonCommon::Broadcaster<void(ObjectRoot*, PlayerId)> OnOwnershipRequest;
		...
		bool HasRealtimeClient() const { return realtimeClient != nullptr; }
		...
		// Binary-safe: length is explicit so callers can pass blobs that
		// contain 0x00 bytes (e.g. 16-byte GUIDs) without the older
		// strlen-based variants truncating at the first interior null.
		Map  MapChange(const PhotonCommon::CharType* data, size_t dataLength);
		Map  MapAdd(const PhotonCommon::CharType* data, size_t dataLength);
