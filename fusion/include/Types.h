...
#pragma pack(pop)
	...
	public:
		...
		virtual ~Object() { EngineBlob.Free(); }
	...
	struct InterestKeySet
	{
		...
		[[nodiscard]] std::vector<std::tuple<uint64_t, uint8_t>> GetAllAreaKeys() const
		{
			...
			{
				if (encoded & 1)
					result.emplace_back(encoded >> 1, sendRate);
			}
			return result;
		}
	...
	struct InputEntry
	{
		...
		InputEntry(InputEntry&& other) noexcept
		    : Sequence(other.Sequence), DeltaTime(other.DeltaTime), Payload(other.Payload), IsNew(other.IsNew), SendCount(other.SendCount)
		{
			other.Payload = {};
		}
	...
	public:
		...
		[[nodiscard]] ObjectOwnerIntent            GetOwnerIntent() const { return OwnerIntent; }
		...
		{
			float total = 0;
			for (const auto& entry : PredictedState.Inputs)
			{
				total += entry.DeltaTime;
			}
			return total;
		}
